...
public class JDialogueIDEUtil {
	...
	public static Project showImportProjectDialog(String title, JDialogueIO io) {
		...
		File f = LWJGUIDialog.showOpenFileDialog(title, getProjectDirectory(), filterDescription, filetype, "");
		...
		if (f != null) {
			try {
				...
				e.printStackTrace();
				LWJGUIDialog.showMessageDialog("Caught " + e.getClass().getName(), e.getMessage(), DialogIcon.ERROR);
			}
		} 
	...
	public static void showExportProjectDialog(Project project, JDialogueIO io) {
		...
		File f = LWJGUIDialog.showSaveFileDialog(title, getProjectDirectory(), filterDescription, filetype, false);
		
		try {
			io.exportProject(project, f);
			LWJGUIDialog.showMessageDialog("Export Success", project.getName() + " was successfully exported.", DialogIcon.INFO);
		} catch (Exception e) {
			e.printStackTrace();
			LWJGUIDialog.showMessageDialog(e.getClass().getName() + " Caught", e.getMessage(), DialogIcon.ERROR);
		}
	}
	...
	public static void showProjectDirectorySelectDialog() {
		...
		File dir = LWJGUIDialog.showOpenFolderDialog("Select Project Directory", defaultLocation.exists() ? defaultLocation : new File(""));
	...
	public static void showSyntaxFileSelectDialog() {
		...
		File file = LWJGUIDialog.showOpenFileDialog("Select Syntax File", defaultLocation.exists() ? defaultLocation : new File(""), "Syntax Text Files", "txt", "");
